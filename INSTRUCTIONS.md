...
### Task 6.3 交付包
- [x] `output/paper/论文完整版.md`（含参考文献列表）
- [x] `output/paper/论文完整版_phase6.docx`（三线表格式化 + 标题格式化）
- [x] `output/tables/`（19 个 CSV 文件）与 `output/figures_v2/`（28 张 300 DPI PNG）
- [x] `notes/` 全部方法与决策说明
- [x] `logs/` 完整执行日志（text + jsonl）
- 说明：`output/paper/论文完整版*` 属可再生产物，当前分支若未跟踪可按 Quick Run List 重建。

### 🛑 FINAL CHECKPOINT
- [x] 论文可提交版本完成（Markdown + DOCX 双版本，参考文献列表已补齐）
- [x] 全部图表与结论可追溯
- [x] 日志与复现材料齐全
- [ ] 页码/页眉/目录需人工在 Word 中最终确认
- 更新于：2026-03-02（Phase6 主体完成，仅余页码页眉人工终检）
...
### 复用基线（默认不重跑）
- 图表：`output/figures_v2/`（28 张 300 DPI）
- 表格：`output/tables/`（现有结果表直接复用）
- 模型：`output/models/rf_model.joblib`、`output/models/gbdt_model.joblib`
- 脚本：`scripts/phase5_prepare.py`、`scripts/phase6_format_tables.py`、`scripts/phase6_audit.py`

### Quick Run List（正文每改一轮执行一次）
1. 仅修改章节草稿（`output/paper/chapter*_draft.md`、`chapter2_lit_review.md`、`abstract_draft.md`）。
2. 运行以下最小命令链（不重训、不重画）：
   ```bash
   cd /Users/mac/Desktop/Grad_thesis
   /Users/mac/miniconda3/bin/python scripts/phase5_prepare.py
   pandoc output/paper/论文完整版.md --reference-doc=商学院本科生毕业论文格式模板2025.01.docx -o output/paper/论文完整版_phase6.docx
   /Users/mac/miniconda3/bin/python scripts/phase6_format_tables.py
   /Users/mac/miniconda3/bin/python scripts/phase6_audit.py
   ```
3. 检查 `notes/phase6_delivery_audit.md` 是否通过，再进行 Word 人工终检（页眉/页码/目录）。
4. 同步更新 `logs/YYYYMMDD/run.log`、`logs/YYYYMMDD/events.jsonl` 与 `memory.md`。
...
- 不重复生成 `output/figures_v2/` 全部 28 张图，除非触发上述重跑条件。
