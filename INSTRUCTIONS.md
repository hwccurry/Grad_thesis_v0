...
7. 最终格式必须对齐 `商学院本科生毕业论文格式模板2025.01.docx`。
8. 每次完成跑数据的程序后，必须同步交付可复现的 Python 或 Stata 程序（含脚本文件路径、执行命令、输入数据路径、输出结果路径）。
9. 每个 Phase 完成后，必须执行 Git 交付：`git add -A`、`git commit -m "phaseX: ..."`、`git push <remote> <branch>`。
10. 第 9 条中的 commit message 必须包含 phase 编号与 comments（本阶段关键修改、关键结果、风险/待办）。
