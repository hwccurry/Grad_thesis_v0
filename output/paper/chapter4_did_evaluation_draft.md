...
**（一）安慰剂检验。** 为排除基准结果由随机因素驱动的可能性，本章进行500次随机政策时间的安慰剂检验：每次随机为每家公司分配一个伪政策年份和伪处理组身份，重新估计DID系数。图12展示了安慰剂系数的核密度分布。

![图12 安慰剂检验（500次模拟）](../figures/did_placebo_test.png)

**图12 安慰剂检验（500次随机模拟）**

如图12所示，安慰剂系数均集中分布在零值附近，呈近似正态分布，而真实DID系数（DivDummy: 0.0828; DivPayRate: 0.1081，红色虚线）远离安慰剂分布的尾部，确认了基准回归捕捉到的是真实的政策效应而非统计噪声。

**（二）PSM-DID。** 为缓解处理组与对照组在可观测特征上的潜在不平衡，本章采用倾向得分匹配（PSM）方法：以全部控制变量为协变量，使用Logit模型估计处理组概率，并进行10近邻匹配（卡尺0.05）。匹配后样本为8,816个观测值。PSM-DID结果显示，DID系数在DivDummy上为0.0828（t=6.802），在DivPayRate上为0.1082（t=6.451），均在1%水平上显著，与基准回归高度一致。
...
| 稳健性检验 | DivDummy did系数 | 显著性 | DivPayRate did系数 | 显著性 |
|-----------|-----------------|--------|-------------------|--------|
| 基准回归 | 0.0828 | *** | 0.1081 | *** |
| PSM-DID | 0.0828 | *** | 0.1082 | *** |
| 排除资本留存（did主效应） | 0.0851 | *** | 0.1099 | *** |
| 排除资产增长（did主效应） | 0.0801 | *** | 0.1044 | *** |
| 剔除再融资样本 | 0.0854 | *** | 0.1078 | *** |

注：\*\*\* p<0.01, \*\* p<0.05, \* p<0.1。数据来源于CSMAR数据库，作者计算。

综合上述稳健性检验，基准回归得出的政策正向因果效应在多种规格和样本口径下均保持稳健。

### 4.4.4 异质性分析

为检验假设H3——政策效应在不同企业群体间的异质性——本章从代理成本、产权性质、机构持股和法治水平四个维度进行分组回归，并通过Wald检验考察组间差异的统计显著性。表10汇报了异质性分析结果。
...
综合异质性分析结果，**假设H3部分得到支持**：代理问题更突出的企业对政策冲击的响应显著更强，且组间差异具有统计显著性。产权性质、机构持股和法治水平维度虽呈现出方向一致的异质性模式，但组间差异未达到常规统计显著水平。
...
**第一**，基准回归结果显示，2023指引显著提升了处理组企业的分红意愿（约8.3个百分点）和分红水平（约10.8个百分点），**假设H2得到支持**。该结果在平行趋势检验、安慰剂检验、PSM-DID、排除替代性解释和剔除再融资样本等多种稳健性检验下均保持稳定。

**第二**，异质性分析表明，政策效应在代理成本高的企业中显著更强（组间差异在10%水平显著），**假设H3部分得到支持**。这一发现从因果推断角度呼应了第三章机器学习分析中代理成本变量（Tunneling）进入特征重要性Top 10的结论——代理问题不仅是预测分红行为的重要因素，也是调节政策效应强度的关键机制。
