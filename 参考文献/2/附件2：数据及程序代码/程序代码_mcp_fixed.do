...
log using 程序代码_mcp_fixed.log, replace text
...
cap which bdiff
if _rc ssc install bdiff, replace
cap which sum2docx
if _rc ssc install sum2docx, replace
...
bdiff, group(law_high) model(cap reghdfe DivPayRate did $ctrl ,absorb(year stkcd) cl(stkcd)  keepsingletons) seed(123) reps(1000) bs
